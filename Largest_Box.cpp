/*------------------------------------
   ...
   created : 16-August,2021 09:47:28 PM
-------------------------------------*/
...
int main() {
    ...
    while (test--) {
        ...
        cin >> h >> w;
        double res = 0.0;
        double l = 0.00, r = (min(h, w)) / 2.00;
        while ((r - l) >= EPS) {
        	double mid1 = l + (r - l) / 3.0;
        	double mid2 = l + ((r - l) / 3.0) * 2.0;
        	double ans1 = mid1 * (h - (2.0 * mid1)) * (w - (2.0 * mid1));
        	double ans2 = mid2 * (h - (2.0 * mid2)) * (w - (2.0 * mid2));
        	if (ans1 >= ans2) r = mid2 - EPS;
        	else l = mid1 + EPS;
        	umax(res, ans1);
        	umax(res, ans2);
        }
